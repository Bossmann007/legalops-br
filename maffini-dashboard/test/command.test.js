...
test('claude_legal command → 501 not_implemented (no egress)', async () => {
  const routes = commandRoutes(makeCfg());
  const res = await run(routes, 'POST /api/command/run', {
...
test('approval record written without raw redactable PII', async () => {
  ...
  // societario_review stays internal/deferred and has a redactable `text` input,
  // so it exercises the approval-record path without invoking the legalops binary.
  const res = await run(routes, 'POST /api/command/run', {
    id: 'societario_review', inputs: { text: secret }, approved: true
  });
  ...
  // Raw PII must never appear in the persisted approval record.
  assert.ok(!stored.includes(secret), 'raw redactable value leaked into store');
  assert.ok(!stored.includes('123.456.789-00'), 'raw CPF leaked');

  const parsed = JSON.parse(stored);
  const rec = parsed.items[0];
  assert.equal(rec.command_id, 'societario_review');
